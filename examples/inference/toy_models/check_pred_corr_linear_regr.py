...
from abc_rejection_sampling import abc_inference, abc_inference_marginal, abc_inference_linear_regression, abc_inference_MLP_regression
...
nnm = CNNModel(input_shape=(ts_len,nr_of_species), output_shape=(15), con_len=3, con_layers=clay,pooling_len=2, dense_layers=[400,400,400],dataname='C')
...
for j in range(15):
    ...
    print(para_names[i], " bins: ", bins[-1])
...
for i in range(0,10+nrs):
    ...
    bin_box_tot.append(bin_box)

    plt.savefig("check_corr_lr_" + str(i))
    plt.close()
