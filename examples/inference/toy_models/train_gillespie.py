...
# ex S-I-S model
channels = 2
transitions = np.array([[-1, 1],[1, -1]])
eq = lambda v: np.array([v[0]*v[1], v[1]])
theta = np.array([1, 20])

init = np.array([18,2])
delta_t, final_t = 0.01,1
...
modelname = "SISmodel" + str(dmin) + "_" + str(dmax)

#Defining true parameter value and generating data from it
true_param = np.array([0.3, 0.4])
GM.theta = true_param
...
for rounds in range(inference_rounds):
    ...
    nnm = CNNModel(input_shape=(ts_len,2), output_shape=(2), con_len=3, con_layers=[25,50])
    ...
    nnm.train(inputs=train_ts, targets=train_thetas,validation_inputs=validation_ts,validation_targets=validation_thetas,
              batch_size=32, epochs=training_epochs, learning_rate=0.001, plot_training_progress=False)
