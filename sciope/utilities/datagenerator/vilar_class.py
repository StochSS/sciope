...
class Vilar:
    ...
    def __init__(self, num_trajectories=1, num_timestamps=501, final_time=100, species=['A','C']):
        ...
        config_file = os.path.dirname(
                                   "sciope/utilities/datagenerator/StochSS_model/vilar_oscillator_AIYDNg/models/data/vilar_oscillator.xml")
        ...
        self.name = 'vilar_ft' + str(self.final_time) + '_ts' + str(self.num_timestamps) + '_tr' + str(self.num_trajectories) \
        + '_species' + str(self.species)
        ...
        # Set tspan
        self.model.tspan = np.linspace(0, self.final_time, self.num_timestamps)
    ...
    def simulate(self, param):
        ...
        if self.species == 'all':
            ...
            species_list = [s for s in self.model.listOfSpecies.keys()]

            s_trajectories = np.array(
                [[simple_trajectories[i][s] for s in species_list] for i in
                 range(self.num_trajectories)])
        else:
            s_trajectories = np.array(
                [[simple_trajectories[i][self.species[j]] for j in
                  range(len(self.species))] for i in range(self.num_trajectories)])
